...
import {
  FakeContractAction,
  FakeContractService,
} from '../integrations/fake-contract.service';
import {
  FakeDocumentAction,
  FakeDocumentService,
} from '../integrations/fake-document.service';
import {
  FakePaymentAction,
  FakePaymentService,
} from '../integrations/fake-payment.service';
...
export class PostSaleService {
  ...
  async overview(schoolId: string): Promise<Record<string, unknown>> {
    ...
    const stateByStudent = new Map(
      states.map((state) => [state.studentKey, state]),
    );
    const eventsByStudent = this.groupByStudent(events);
    const lifecycle = baseLifecycle.map((student) => {
      const withState = this.applyState(
        student,
        stateByStudent.get(student.id),
      );
  ...
  async studentProfile(
    ...
    const [enrollment, storedEvents, storedTasks, logs] = await Promise.all([
      ...
      this.integrationLogs.forStudent(
        schoolId,
        student.id,
        student.enrollmentId,
        60,
    ...
    const documentRequirements = this.documentRequirementRows(
      student,
      enrollment,
      runtimeConfig,
      storedEvents,
    ...
    return {
      ...
      documents: {
        ...
        uploaded: (enrollment?.documents ?? []).map((document) =>
          this.serializeEnrollmentDocument(document),
        ),
  ...
  async updateStudentStatus(
    ...
    await this.recordEvent(schoolId, student, {
      ...
      description:
        input.note?.trim() || this.actionDescription(action, student),
      metadata: { action },
    });
  ...
  async simulateMessage(
    ...
    const student = await this.findStudent(schoolId, studentKey);
    const runtimeConfig = await this.schoolConfig.getRuntimeConfig(schoolId);
    const message = this.cleanVisibleText(
      input.message?.trim() || this.suggestedMessage(student, runtimeConfig),
      student,
    );
  ...
  async simulateRuler(
    ...
    const student = await this.findStudent(schoolId, studentKey);
    const runtimeConfig = await this.schoolConfig.getRuntimeConfig(schoolId);
    const template = this.pickRulerTemplate(
      student,
      runtimeConfig,
      input.dayOffset,
    );
    if (!template) {
      throw new BadRequestException(
        'Nenhum template ativo da régua foi encontrado para este aluno.',
      );
    }

    const message = this.schoolConfig.renderTemplate(
      template.whatsappText,
      this.studentVariables(student, runtimeConfig),
    );
    const whatsapp = await this.fakeWhatsApp.sendMessage({
      context: this.integrationContext(schoolId, student),
      message,
  ...
  async simulatePayment(
    ...
    await this.saveStatePatch(
      schoolId,
      student,
      this.paymentPatch(action, student, result.paymentStatus),
    );
    await this.recordEvent(schoolId, student, {
      type: 'PAGAMENTO_FAKE',
  ...
  async simulateContract(
    ...
    await this.saveStatePatch(
      schoolId,
      student,
      this.contractPatch(action, student, result.contractStatus),
    );
    await this.recordEvent(schoolId, student, {
      type: 'CONTRATO_FAKE',
  ...
  async simulateDocument(
    ...
    const fileName =
      input.fileName?.trim() || this.fakeDocumentFileName(documentType);
    ...
    await this.saveStatePatch(
      schoolId,
      student,
      this.documentPatch(action, student, result.documentStatus, documentType),
    );
    await this.recordEvent(schoolId, student, {
      type: 'DOCUMENTO_FAKE',
  ...
  private fromEnrollment(enrollment: any, index: number): LifecycleStudent {
    ...
    const daysSinceEnrollment = this.daysBetween(
    ...
    else if (daysSinceEnrollment >= 10 && index % 4 === 0)
      status = 'RISCO_EVASAO';
    ...
    const progress = Math.round(
      (checklist.filter((step) => step.status === 'done').length /
        checklist.length) *
        100,
    );
  ...
  private demoStudents(): LifecycleStudent[] {
    ...
    return demos.map((demo, index) => ({
      ...
      lastContactAt: this.addDays(
        new Date(),
        -Math.min(demo.daysSinceEnrollment, 5),
      ),
  ...
  private checklist(input: {
    ...
    return [
      ...
      },
      {
        ...
        status: input.hasContract
          ? 'done'
          : input.hasMinimumDocs
            ? 'pending'
            : 'blocked',
        helper:
          'Integração futura com D4Sign para gerar e acompanhar assinatura.',
      },
      {
        key: 'pagamento',
      ...
      },
      {
        ...
        status: input.risk
          ...
          : input.daysSinceEnrollment >= 7
            ...
            : 'pending',
        helper:
          'Checagem de primeiro acesso, progresso e dificuldade do aluno.',
      },
    ];
  }
  ...
  private demoChecklist(status: LifecycleStatus): ChecklistStep[] {
    return this.checklist({
      ...
      hasContract: !['DOCUMENTACAO_PENDENTE', 'CONTRATO_PENDENTE'].includes(
        status,
      ),
  ...
  private applyState(student: LifecycleStudent, state?: any): LifecycleStudent {
    ...
    next.progress = Math.round(
      (next.checklist.filter((step) => step.status === 'done').length /
  ...
  private profileCourse(
    ...
    const course =
      config.courses.find((item) => item.name === student.course) ?? null;
    ...
    const rows = [
      {
        ...
        label: 'Nome completo',
        value: enrollment?.studentName ?? student.studentName,
      },
      {
        section: 'Identificação',
        label: 'Documento',
        value: this.documentLabel(enrollment, data),
      },
      {
        section: 'Identificação',
        label: 'País de residência',
        value: enrollment?.countryOfResidence ?? data.countryOfResidence,
      },
      {
        section: 'Identificação',
        label: 'Idioma preferido',
        value: enrollment?.preferredLanguage ?? data.preferredLanguage,
      },
      {
        section: 'Contato',
        label: 'E-mail',
        value: enrollment?.email ?? data.email,
      },
      {
        section: 'Contato',
        label: 'Telefone',
        value: enrollment?.phone ?? data.phone,
      },
      { section: 'Dados pessoais', label: 'Nascimento', value: data.birthDate },
      {
        section: 'Dados pessoais',
        label: 'Sexo',
        value: data.gender ?? data.sexo,
      },
      {
        section: 'Dados pessoais',
        label: 'Nacionalidade',
        value: data.nationality ?? data.nacionalidade,
      },
  ...
  private documentRequirementRows(
    ...
    return config.documents
      ...
      .map((requirement) => {
        const uploaded = uploadedDocuments.find((document) =>
          this.sameDocumentType(document.type, requirement.documentType),
        );
  ...
  private documentFakeStates(storedEvents: any[]) {
    ...
    for (const event of storedEvents) {
      ...
      const status =
        String(metadata.documentStatus ?? '').trim() ||
        this.statusFromDocumentAction(String(metadata.action ?? ''));
  ...
  private documentAudiences(student: LifecycleStudent, enrollment: any | null) {
    ...
    const isInternational =
      /passaporte|passport|ssn|driver|state id|nie|dni|estrangeir|internacional/i.test(
        docType,
      ) ||
      (!!country && !/brasil|brazil/i.test(country));
    const audiences = [isInternational ? 'estrangeiro' : 'brasileiro'];
    const age = data.birthDate
      ? this.ageFromBirthDate(String(data.birthDate))
      : null;
  ...
  private profileTimeline(
    ...
    for (const task of storedTasks) {
      entries.push({
        ...
        type: 'TASK_CREATED',
        title:
          task.status === 'ABERTA' ? 'Tarefa em aberto' : 'Tarefa registrada',
  ...
  private profileActions(student: LifecycleStudent, storedTasks: any[]) {
    const recommended = {
      ...
      origin: 'ia',
      createdBy: 'automacao',
      relatedEntity: { rule: 'recomendacao' },
      autoResolve: true,
      dueAt: student.upcomingDueAt,
      automation: this.automationFor(student.status),
      status:
        student.status === 'ONBOARDING_CONCLUIDO' ? 'CONCLUIDA' : 'RECOMENDADA',
      source: 'automatic',
    };
    ...
    return tasks.length ? tasks : [recommended];
  }
  ...
  private profileRisk(
    ...
    const pendingRequired = documents.filter(
      (document) => document.required && document.status === 'PENDENTE',
    );
    const rejected = documents.filter(
      (document) => document.status === 'RECUSADO',
    );
  ...
  private riskReasons(
    ...
    if (student.status === 'DOCUMENTACAO_PENDENTE')
      reasons.push('Documentação ainda bloqueia contrato e acesso.');
    if (pendingRequired.length)
      reasons.push(
        `${pendingRequired.length} documento(s) obrigatório(s) ainda estão pendentes.`,
      );
    if (rejected.length)
      reasons.push(
        `${rejected.length} documento(s) recusado(s) precisam ser reenviados.`,
      );
    if (student.status === 'CONTRATO_PENDENTE')
      reasons.push('Contrato não está concluído.');
    if (student.status === 'PAGAMENTO_PENDENTE')
      reasons.push('Pagamento da matrícula está pendente.');
    if (student.status === 'ACESSO_PENDENTE')
      reasons.push('Acesso ao AVA ainda não foi liberado.');
    if (student.status === 'RISCO_EVASAO')
      reasons.push('Aluno precisa de intervenção de permanência.');
    if (student.ruler.pendingCount > 0)
      reasons.push(
        `${student.ruler.pendingCount} mensagem(ns) da régua estão pendentes.`,
      );
    if (student.daysSinceEnrollment >= 7 && student.progress < 80)
      reasons.push('Jornada com mais de 7 dias e avanço abaixo do esperado.');
  ...
  private contractPatch(
    action: FakeContractAction,
    student: LifecycleStudent,
    contractStatus: string,
  ) {
    switch (action) {
  ...
  private actionPatch(
    action: PostSaleAction | undefined,
    student: LifecycleStudent,
  ) {
    ...
    switch (action) {
      ...
      case 'CONTRACT_SIGNED':
        return {
          ...
          nextAction:
            student.paymentStatus === 'Pago'
              ? 'Liberar acesso ao AVA'
              : 'Enviar lembrete de pagamento',
          ownerTeam:
            student.paymentStatus === 'Pago' ? 'Suporte AVA' : 'Financeiro',
          riskScore: Math.min(student.riskScore, 32),
        };
      case 'PAYMENT_PAID':
        return {
          status: /assinado/i.test(student.contractStatus)
            ? 'ACESSO_PENDENTE'
            : 'CONTRATO_PENDENTE',
          paymentStatus: 'Pago',
          nextAction: /assinado/i.test(student.contractStatus)
            ? 'Liberar acesso ao AVA'
            : 'Acompanhar assinatura do contrato',
          ownerTeam: /assinado/i.test(student.contractStatus)
            ? 'Suporte AVA'
            : 'Secretaria',
      ...
      case 'RISK_RESOLVED':
        return {
          status: 'EM_ACOMPANHAMENTO',
          accessStatus: /sem primeiro acesso/i.test(student.accessStatus)
            ? 'Liberado'
  ...
  private timelineFor(
    ...
    const base: TimelineEvent[] = [
      ...
      },
      {
        ...
        description:
          'Mensagem inicial com documentos, contrato, pagamento e acesso ao AVA.',
  ...
  private summary(students: LifecycleStudent[]) {
    return {
      ...
      documentsPending: students.filter(
        (student) => student.status === 'DOCUMENTACAO_PENDENTE',
      ).length,
      contractsPending: students.filter(
        (student) => student.status === 'CONTRATO_PENDENTE',
      ).length,
      paymentsPending: students.filter(
        (student) => student.status === 'PAGAMENTO_PENDENTE',
      ).length,
      accessPending: students.filter(
        (student) => student.status === 'ACESSO_PENDENTE',
      ).length,
      highRisk: students.filter((student) =>
        ['ALTO', 'CRITICO'].includes(student.riskLevel),
      ).length,
      onboardingComplete: students.filter(
        (student) => student.status === 'ONBOARDING_CONCLUIDO',
      ).length,
      automationsQueued: students.reduce(
        (sum, student) => sum + student.ruler.pendingCount,
        0,
      ),
  ...
  private automations(
    config: RuntimeSchoolConfig,
    students: LifecycleStudent[],
  ) {
    return this.rulerTemplates(config).map((template) => ({
      day: template.dayOffset ?? 0,
      title: template.title,
      channel: 'WhatsApp',
      trigger: template.stage,
      message: this.schoolConfig.renderTemplate(
        template.whatsappText,
        this.schoolConfig.defaultVariables(config),
      ),
      status: template.active ? 'Ativa' : 'Inativa',
      sentCount: students.filter((student) =>
        student.ruler.sentDays.includes(template.dayOffset ?? 0),
      ).length,
      pendingCount: students.filter(
        (student) =>
          student.ruler.nextDay === template.dayOffset &&
          student.ruler.status === 'PENDENTE',
      ).length,
      scheduledCount: students.filter(
        (student) =>
          student.ruler.nextDay === template.dayOffset &&
          student.ruler.status === 'AGENDADA',
      ).length,
    }));
  }
  ...
  private rulerTemplates(config: RuntimeSchoolConfig) {
    return config.templates
      .filter(
        (template) =>
          template.category === 'regua' && template.dayOffset !== null,
      )
  ...
  private rulerStatus(
    ...
    const sentDays = [...new Set(sentEntries.map((entry) => entry.day))].sort(
      (a, b) => a - b,
    );
    const unsentTemplates = templates.filter(
      (template) => !sentDays.includes(template.dayOffset ?? -1),
    );
    const dueTemplates = unsentTemplates.filter(
      (template) => (template.dayOffset ?? 0) <= student.daysSinceEnrollment,
    );
    ...
    return {
      ...
      nextMessage: nextTemplate
        ? this.schoolConfig.renderTemplate(
            nextTemplate.whatsappText,
            this.studentVariables(student, config),
          )
        : null,
      sentDays,
  ...
  private pickRulerTemplate(
    ...
    if (typeof requestedDay === 'number' && Number.isFinite(requestedDay)) {
      return (
        templates.find((template) => template.dayOffset === requestedDay) ??
        null
      );
    }
    if (typeof student.ruler.nextDay === 'number') {
      return (
        templates.find(
          (template) => template.dayOffset === student.ruler.nextDay,
        ) ?? null
      );
    }
  ...
  private messageTemplates(config: RuntimeSchoolConfig) {
    return config.templates
      ...
      .map((template) => ({
        title: template.title,
        text: this.schoolConfig.renderTemplate(
          template.whatsappText,
          this.schoolConfig.defaultVariables(config),
        ),
      }));
  ...
  private suggestedMessage(
    ...
    const template = this.schoolConfig.templateByKey(
      config,
      keyByStatus[student.status] ?? 'welcome',
    );
    if (template) {
  ...
  private studentVariables(
    student: LifecycleStudent,
    config: RuntimeSchoolConfig,
  ) {
    const course =
      config.courses.find((item) => item.name === student.course) ??
      config.courses[0];
  ...
  private documentLabel(enrollment: any | null, data: Record<string, unknown>) {
    const type =
      enrollment?.documentType ??
      data.documentType ??
      (enrollment?.cpf ? 'CPF' : null);
    const number =
      enrollment?.documentNumber ?? data.documentNumber ?? enrollment?.cpf;
  ...
  private roleForTeam(team: string) {
    ...
    if (normalized.includes('sucesso') || normalized.includes('retencao'))
  ...
  private riskScore(status: LifecycleStatus, daysSinceEnrollment: number) {
    ...
    return Math.min(
      100,
      base[status] + Math.max(0, daysSinceEnrollment - 7) * 2,
    );
  }
